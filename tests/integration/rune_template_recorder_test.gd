...
const RECORDER_SCENE := preload("res://tools/authoring/rune_template_recorder.tscn")
...
func _init() -> void:
	...
	if not _require(vertical_lock == Vector2(0.25, 0.9), "Shift lock should preserve the anchor x for vertical lines."):
		return
	var diagonal_input := Vector2(0.45, 0.55)
	var diagonal_lock := recorder.call("_axis_locked_point", diagonal_input) as Vector2
	var diagonal_distance_mm := maxf(
		absf((diagonal_input.x - 0.25) * recorder.recorder_canvas_size_mm.x),
		absf((diagonal_input.y - 0.25) * recorder.recorder_canvas_size_mm.y))
