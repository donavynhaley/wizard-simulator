...
@export var bob_amplitude_y := 0.006
@export var bob_amplitude_x := 0.004
## Slight roll rocked in sync with the stride.
@export var bob_roll := 0.012
...
@export var breath_amplitude := 0.0028
...
func _update_look_sway(delta: float) -> void:
	...
	var yaw_vel := wrapf(yaw - _prev_yaw, -PI, PI) / delta
	var pitch_vel := (pitch - _prev_pitch) / delta
	_prev_yaw = yaw
	_prev_pitch = pitch
